...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class RemoteHeadManager : Singleton<RemoteHeadManager>
    {
        ...
        private void UpdateHeadTransform(NetworkInMessage msg)
        {
            ...
            GameObject [] spawners = GameObject.FindGameObjectsWithTag("butterflyspawner");
            //Debug.Log("Found " + spawners.Length + " butterflies and setting the otherUserPos to " + headPos);
            /**
             * This grabs the location from the other user and the finds that location relative to the main 
             * camera and passes it to the spawn object to use later
             */
            if(spawners.Length > 0)
            {
                for (int i = 0; i < spawners.Length; i++)
                {
                    ...
                    spawners[i].GetComponent<Spawner>().SetOtherUserPos(headPos);
                }
            }
           
            //GetComponent<ButterflyBehavior>().otherUserPos = headPos;
        ...
        private GameObject CreateRemoteHead()
        {
            ...
            newHeadObj.transform.localScale = Vector3.one * 0.1f;
            return newHeadObj;
        }
